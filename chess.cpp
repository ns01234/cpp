...
bool checkMove(Piece* board[8][8], string src, string dest){
    char a = src[1] - '1';
    char b = src[0] - 'a';
    char i = dest[1] - '1';
    char j = dest[0] - 'a';
...
void check_checker(string side, Piece* wkingptr, Piece* bkingptr){
...
void move(Piece* board[8][8], string src, string dest, Piece* wkingptr, Piece* bkingptr){
    ...
    if(board[i][j]->color == "white" && board[k][l]->dangerb){
        board[a][b] = board[i][j];  
        board[i][j] = hold;
        ...
        cout << "to where: ";
        cin >> dest;
        if(checkMove(board, src, dest)){
            move(board, src, dest, wkingptr, bkingptr);
            return;
        }
    }

    if(board[i][j]->color == "black" && board[m][n]->dangerw){
        board[a][b] = board[i][j];
        board[i][j] = hold;
        board[a][b]->coordinate[0] = src[0];
        board[a][b]->coordinate[1] = src[1];
...
int main() {
    ...
    while(src != "x"){
        ...
        check_checker(side[turn % 2], wkingptr, bkingptr);
        turn += 1;
        printboard(board);
